...
type GameState = {
    ...
    gyroCalibration: { x: number; y: number };
};
...
const bestElement = document.getElementById('best') as HTMLSpanElement;
...
function drawMaze() {
    ...
    if (state.player.vx !== 0 || state.player.vy !== 0) {
        ...
        ctx.lineTo(
            state.player.x + state.player.vx * 20,
            state.player.y + state.player.vy * 20
        );
        ctx.stroke();
    }
}
...
function movePlayer(deltaTime: number) {
    ...
    const distance = Math.sqrt(
        Math.pow(state.player.x - state.target.x, 2) + 
